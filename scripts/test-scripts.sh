...
assert_contains "${docker_scan_full_output}" "scope: project-owned runtime images and local infrastructure images"
...
assert_contains "${dependency_freshness_output}" "Maven direct dependency updates:"
assert_contains "${dependency_freshness_output}" "Maven dependency-management updates:"
assert_contains "${dependency_freshness_output}" "Maven plugin updates:"
assert_contains "${dependency_freshness_output}" "npm reported outdated dependencies above"
assert_contains "${dependency_freshness_output}" "mysql:8.4"
assert_contains "${dependency_freshness_output}" "pinned tag: 8.4"
...
assert_contains "${freshness_mvn_log}" "versions:display-dependency-updates -DprocessDependencyManagement=false"
assert_contains "${freshness_mvn_log}" "versions:display-dependency-updates -DprocessDependencies=false -DprocessDependencyManagement=true"
